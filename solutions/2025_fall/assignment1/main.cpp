...
void parse_csv(const std::string& filename, std::vector<Course>& courses) {
  ...
  while (std::getline(file, line)) {
    ...
    courses.push_back(Course{entry[0], entry[1], entry[2]});
  }
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
  // Write to the file "student_output/courses_offered.csv"
  ...
  // Write the offered courses
  for (const auto& course :  all_courses) {
    if (course.quarter != "null") {
      file << course.title << "," << course.number_of_units << "," << course.quarter << "\n";
      indices_to_delete.push_back(&course - &all_courses[0]);
    }
  }

  // Reverse sort the indices to delete to avoid invalidating indices
  std::reverse(indices_to_delete.begin(), indices_to_delete.end());
  // Delete the offered courses from all_courses
  for (const auto& index : indices_to_delete) {
    delete_elem_from_vector(all_courses, all_courses[index]);
  }
}
